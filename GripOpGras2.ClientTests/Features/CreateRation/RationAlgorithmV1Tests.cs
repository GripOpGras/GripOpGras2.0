...
namespace GripOpGras2.ClientTests.Features.CreateRation
{
	public class RationTests
	{
		...
		public void TestTotalCalculations(float vem, float vemSupplementaryFeedProduct, float appliedVem,
			float appliedVeMbijpord,
			...
			float expectedDm, float expectedDmSupplementaryFeedProduct, float expectedRediff)
		{
			...
			ration.ApplyChangesToRationList(GetMappedFeedProduct("product 2", reSupplementaryFeedProduct,
				vemSupplementaryFeedProduct,
			...
			Assert.AreEqual(expectedVemSupplementaryFeedProduct, ration.TotalVemSupplementaryFeedProduct,
				"total vem SupplementaryFeedProduct");
			...
			Assert.AreEqual(expectedDmSupplementaryFeedProduct, ration.TotalDmSupplementaryFeedProduct,
				"total dm SupplementaryFeedProduct");
			Assert.AreEqual(expectedRediff, ration.TotalReDiff, "total rediff");
		}

		[Test]
		[TestCase(1000, 500, 500, 150, 200, 500, 250, 0.75f, 0.5f, 25)]
		public void TestAllCalculationsWithCombos(float vem, float vemSupplementaryFeedProduct, float appliedVem,
			float re,
			float reSupplementaryFeedProduct, float expectedVem, float expectedVemSupplementaryFeedProduct,
			float expectedDm, float expectedDmSupplementaryFeedProduct,
			float expectedRediff)
		{
			RationPlaceholder ration = new();
			MappedFeedProduct mappeditem1 = GetMappedFeedProduct("product 1", re, vem, 5);
			MappedFeedProduct mappeditem2 = GetMappedFeedProduct("product 2", reSupplementaryFeedProduct,
				vemSupplementaryFeedProduct, 5, false);
	...
	public class RationAlgorithmV1Tests
	{
		...
		public async Task CreateRationAsyncTest(float prod1Re, float prod1Vem, float prod2Re, float prod2Vem,
			float supplementaryFeedProductRe, float supplementaryFeedProductVem, float totalgrassIntake, float grassVem,
			float grassRe)
		{
			// Arrange
			...
			FeedProduct prod3 =
				GetFeedProduct("bijprod", supplementaryFeedProductRe, supplementaryFeedProductVem, false);
			Herd herd = new()
			{
				Name = "Herd1",
				NumberOfAnimals = 100,
		...
		public void GetGrassReNuturalizerFeedProductTest(float rEperDMprod1, float vemPerDMprod1, float rEperDMprod2,
			...
			float rEgrass, string expectedProd)
		{
			//Arrange
			...
			FeedProduct supplementaryFeedProduct = GetFeedProduct("bijprod", rEperDmSupplementaryFeedProduct,
				vemPerDmSupplementaryFeedProduct, false);
			RationAlgorithmV1 rationAlgorithm =
				CreateRationAlgorithm(new List<FeedProduct> { prod1, prod2, supplementaryFeedProduct },
					totalGrassIntake: 700,
		...
		public void GenerateReNaturalFeedProductGroupsTest()
		{
			...
			RationAlgorithmV1 ration = CreateRationAlgorithm(new List<FeedProduct> { prod1, prod2, prod3, prod4 });
			//Act
	...
	public class RationAlgorithmV1Tests_testplan
	{
		...
		public void TestMaxKgDmSupplementeryProductIsRespected()
		{
			...
			{
				GetFeedProduct("prod1", 161, 50),
				GetFeedProduct("prod2", 82, 50),
		...
		public void TestAlgorithmImprovementSelection(bool productBIsHalfRoughage, float VemToChange,
			float expectedVemImprovementA,
			float expectedVemImprovementB)
		{
			...
			AbstractMappedFoodItem GroupB = (productBIsHalfRoughage)
				? new MappedFeedProductGroup((prodBa, 1), (prodBb, 1))
				: new MappedFeedProductGroup((prodBa, 1));
			...
			List<AbstractMappedFoodItem> changesPerVemMethodA = new();
			...
			List<AbstractMappedFoodItem> changesPerVemMethodB = new();
			...
			List<AbstractMappedFoodItem> availablFoodGroups = new()
			...
			Console.WriteLine(
				$"current: VEM =  {currentration.TotalVem}, targetVEM = {targetValues.TargetedVem}, currentDM = {currentration.TotalDm}, targetDM = {targetValues.TargetedMaxKgDm}");
			List<AbstractMappedFoodItem> finalchanges =
				selector.DetermineImprovementRationsWithSupplementaryFeedProduct(availablFoodGroups, availablFoodGroups,
					improvementMethods: new ImprovementRationMethodNaturalReGroups());
			...
			AbstractMappedFoodItem? resultGroupA =
				finalchanges.FirstOrDefault(x => x.OriginalReference == changeGroupA.OriginalReference);
			AbstractMappedFoodItem? resultGroupB =
				finalchanges.FirstOrDefault(x => x.OriginalReference == changeGroupB.OriginalReference);
		...
		public void TestImprovementRationWithoutBetterSupplementeryProducts()
		{
			...
			List<FeedProduct> products = new()
			{
				...
				GetFeedProduct("prod2", 82, 960),
				GetFeedProduct("bijprod1", 95, 800, false),
				GetFeedProduct("bijprod1", 117.14f, 801, false),
				GetFeedProduct("bijprod1", 180, 802, false)
			};
			RationAlgorithmV1 algorithm = new();
			//Act
			algorithm.SetUp(products, herd, 13, getmilkProductionAnalysis(), grazingActivity(180, 50));
		...
		public void TestProductREValues(float prod1Re, float prod2Re, float prod3Re, bool shouldThrowExcption)
		{
			...
			List<FeedProduct> products = new()
			{
				GetFeedProduct("prod1", prod1Re, 1037),
				GetFeedProduct("prod2", prod2Re, 960),
				GetFeedProduct("bijprod1", prod3Re, 1219, false)
			};
			RationAlgorithmV1 algorithm = new();
			//Assert
		...
		public void TestTooMuchVEMForGrass()
		{
			...
			RationAlgorithmV1 algorithm = new();
			//Act
			algorithm.SetUp(products, herd, 400, getmilkProductionAnalysis(),
				grazingActivity: grazingActivity(re: 210));
			algorithm.RunAlgorithm();
			//Assert
